...
function [im_out, psnr_out] = RunFP(y, ForwardFunc, BackwardFunc,...
    InitEstFunc, input_sigma, params, orig_im, denoising_eng)
...
for k = 1:1:outer_iters    
    ...
    switch denoising_eng
        case 'tnrd'                
            f_x_est = Denoiser(x_est, effective_sigma);
        case 'mat_DnCNN'                
            net = denoisingNetwork('DnCNN');
            f_x_est = denoiseImage(x_est, net);
        case 'Med3'
            f_x_est = medfilt2(x_est,[3,3]);
        case 'Wnr'
            f_x_est = wiener2(x_est, [5 5]);
    end  
